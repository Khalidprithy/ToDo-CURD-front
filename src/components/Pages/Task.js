...
const Task = ({ task, handleDelete, refetch, isLoading }) => {
    ...
    return (
        <div className='flex justify-center items-center gap-2 py-2 hover:scale-100 duration-300 transform 0.3s transition ease-in-out delay-250 hover:-translate-y-2'>
            ...
            <h4 className={`border-2 p-1 rounded-md w-6/12 text-left text-xl hover:shadow-md ${task?.status ? 'text-green-400 blur-[1px]' : 'text-gray-800'}`}>{task.task}</h4>
            {
                !task?.status &&
